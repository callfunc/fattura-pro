   ...
   Begin TabDlg.SSTab Schedario 
      ...
      Begin VB.PictureBox PannelloBilancio 
         ...
         Begin VB.ComboBox ElencoAnniIncassiSpese 
            Height          =   345
            ItemData        =   "IncassiSpese.frx":03C2
            Left            =   6495
            List            =   "IncassiSpese.frx":03C4
         ...
         Begin VB.ListBox ElencoCF 
            Height          =   2985
            ItemData        =   "IncassiSpese.frx":03C6
            Left            =   165
            List            =   "IncassiSpese.frx":03C8
      ...
      Begin VB.PictureBox PannelloBilancio 
         ...
         Begin VB.ListBox ElencoAnniBilancio 
            Height          =   1185
            ItemData        =   "IncassiSpese.frx":0906
            Left            =   855
            List            =   "IncassiSpese.frx":0908
            TabIndex        =   2
            Top             =   225
            Width           =   990
         End
...
" GROUP BY Format(Data, ""mmmm"") ORDER BY Format(Data, ""mmmm"") ASC", conn, adOpenDynamic
...
& "BY Format(Data, ""mmmm"") ORDER BY Format(Data, ""mmmm"") ASC", conn, adOpenDynamic
  ...
  With TotaliBilancio
   ...
   If Not rsTotaliAcquisti.EOF Then
    If rsTotaliAcquisti("Mese") = LCase(.TextMatrix(i, 0)) Then
     .TextMatrix(i, 2) = FormatNumber(rsTotaliVendite("TotImpAcq"), 2)
   ...
   If Not rsTotaliRimborsi.EOF Then
    If rsTotaliRimborsi("Mese") = LCase(.TextMatrix(i, 0)) Then
     .TextMatrix(i, 2) = CDbl(.TextMatrix(i, 2)) + rsTotaliRimborsi("TotImpRim")
     .TextMatrix(i, 4) = CDbl(.TextMatrix(i, 4)) + rsTotaliRimborsi("TotIvaRim")
    End If
   End If
...
Else
 MsgBox "Attenzione, Dati Bilancio non caricati !", vbExclamation, "Fattura Pro"
End If
End Sub
...
Else
 ...
 Tabella = "Fornitori"
End If
rsDitte.open Tabella, conn, adOpenDynamic
...
End If
End Sub
Private Sub MostraIncassiSpeseMesi(ByVal Ditta As String, Anno As String)
Dim ElencoTotali As MSXML2.IXMLDOMNodeList, fso As New FileSystemObject, TotaleMese#, StrRic$
If TipoDitta(0).Value Then
 If fso.FileExists("Backup\FattureClienti_" & AnniIncassiSpese.Text & ".xml") Then
  DocDati.Load "Backup\FattureClienti_" & AnniIncassiSpese.Text & ".xml"
 Else
  Set DocDati = DocFattureClienti
 End If
 StrRic = "/FattureClienti/FatturaCliente[contains(@cliente," & Chr(34) & Ditta & Chr(34) & ") " _
 & "and substring(@data,4,2)=mese and substring(@data,7,4)=" & Anno & "]/@timp"
Else
 If fso.FileExists("Backup\FattureFornitori_" & AnniIncassiSpese.Text & ".xml") Then
  DocDati.Load "Backup\FattureFornitori_" & AnniIncassiSpese.Text & ".xml"
 Else
  Set DocDati = DocFattureFornitori
 End If
 StrRic = "/FattureFornitori/FatturaFornitore[contains(@fornitore," & Chr(34) & Ditta & Chr(34) _
 & ") and substring(@data,4,2)=mese and substring(@data,7,4)=" & Anno & "]/@timp"
End If
GrigliaIncassiSpese.AddItem "": GrigliaIncassiSpese.TextMatrix(GrigliaIncassiSpese.Rows - 1, 0) = Ditta
For i = 1 To 12
 Set ElencoTotali = DocDati.selectNodes(Replace(StrRic, "mese", CStr(i)))
 For j = 0 To ElencoTotali.Length - 1
  TotaleMese = TotaleMese + ElencoTotali(j).nodeValue
 Next j
 GrigliaIncassiSpese.TextMatrix(GrigliaIncassiSpese.Rows - 1, i) = FormatNumber(TotaleMese, 2): TotaleMese = 0
Next i
End Sub
